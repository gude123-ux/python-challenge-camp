...
npm run test:smoke     # 核心逻辑冒烟测试（95 项）
...
**「抢救」是什么意思？** 如果模型返回的 JSON 结构坏了（比如上面那种拼接），
插件会**逐个字段解析**，把损坏点之前完整的字段捞出来 ——
分数、可运行性、总评这些关键字段都排在前面，所以通常能救回来，不必退化成本地评分。
抢救成功时结果里会带一条提示：「结构损坏，建议列表可能不完整，必要时可重新提交」。

> 历史版本里修过两个解析相关的 bug，都加了回归测试：
> 1.0.1 —— 提示词要求 AI 在建议里给代码片段，于是**合法 JSON 的字符串内部会包含 ` ``` `**，
> 解析器用非贪婪正则剥外层围栏时被内层代码块骗到；
> 1.0.2 —— 上游把两份生成拼接在一起导致结构损坏，改为逐字段抢救。
